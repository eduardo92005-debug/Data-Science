...
def aeroportoCadaCidade(df):
    unique_cidades = df['CIDADE'].unique()
    result_df = None
    for cidade in unique_cidades:
        query = df[(df['CIDADE'] == cidade) & (df['CATEGORIA'] == 'Aeroportos') & (df['QTDE_VIAGENS'] > 1000)].head(1)
    ...
    where post_num=1 and categoria = 'Aeroportos'
